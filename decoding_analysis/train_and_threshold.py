...
def main():
	...
	parser.add_argument('--test_noise', type=float, default=2.0)
	...
	for s in range(signal_test_vals.shape[0]):
		test_acc = test(args, encoder, class_out, conf_out, device, test_loader, signal=signal_test_vals[s], noise=args.test_noise)
	...
	check_path(model_dir)
	np.savez(model_dir + 'threshold_results.npz',
