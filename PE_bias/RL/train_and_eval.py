...
def test(args, encoder, actor, critic, device, gabors, signal=1.0, noise=0.0):
	...
	for batch_idx in range(args.N_test_batches):
		...
		p_a = actor(z)
		v = critic(z).squeeze()
		# Accuracy (ignoring opt-out)
		test_acc = (p_a.probs[:,:2].argmax(1) == y).float().mean().item() * 100.0
		all_test_acc.append(test_acc)
		# Opt-out rate
		opt_out_rate = (p_a.probs.argmax(1) == 2).float().mean().item() * 100.0
		all_test_opt_out_rate.append(opt_out_rate)
	# Overall test accuracy and opt-out-rate
	avg_test_acc = np.array(all_test_acc).mean()
	avg_opt_out_rate = np.array(all_test_opt_out_rate).mean()
	# Report
	log.info('[Signal = ' + '{:.2f}'.format(signal) + '] ' + \
			 '[Noise = ' + '{:.2f}'.format(noise) + '] ' + \
			 '[Class. Acc. = ' + '{:.2f}'.format(avg_test_acc) + '] ' + \
			 '[Opt-out rate = ' + '{:.2f}'.format(avg_opt_out_rate) + ']')
	return avg_test_acc, avg_opt_out_rate
...
def main():
	...
	for n in range(noise_test_vals.shape[0]):
		...
		for s in range(signal_test_vals.shape[0]):
			test_acc, test_opt_out_rate = test(args, encoder, actor, critic, device, gabors, signal=signal_test_vals[s], noise=noise_test_vals[n])
