...
def plot(args):
	...
	for r in range(args.N_runs):
		...
		low_PE_test_conf_correct = test_results['low_PE_test_conf_correct'] / 100
		low_PE_test_conf_incorrect = test_results['low_PE_test_conf_incorrect'] / 100
		high_PE_test_acc = test_results['high_PE_test_acc'] / 100
		high_PE_test_conf_correct = test_results['high_PE_test_conf_correct'] / 100
		high_PE_test_conf_incorrect = test_results['high_PE_test_conf_incorrect'] / 100
	...
	# Legend
	plt.legend(['Correct','Incorrect'],frameon=False,fontsize=axis_label_font_size,bbox_to_anchor=(1,1.08))
	# Significance
	min_y_val = np.min([low_PE_test_conf_correct.min(),high_PE_test_conf_correct.min()])
	y_start = min_y_val - 0.01
	y_end = min_y_val - 0.015
	ax.plot([0,0,1,1],[y_start,y_end,y_end,y_start],color='black')
	ax.text(0.5,y_end-0.04,conf_correct_p_symb,fontsize=significance_font_size,horizontalalignment='center')
	min_y_val = np.min([low_PE_test_conf_incorrect.min(),high_PE_test_conf_incorrect.min()])
	y_start = min_y_val - 0.01
	y_end = min_y_val - 0.015
	ax.plot([2.5,2.5,3.5,3.5],[y_start,y_end,y_end,y_start],color='black')
	ax.text(3,y_end-0.04,conf_incorrect_p_symb,fontsize=significance_font_size,horizontalalignment='center')
	# Title
	plt.title('PE bias (v2) - CIFAR-10',fontsize=title_font_size)
	# Save plot
	plt.tight_layout()
	plot_fname = './PE_bias_v2_CIFAR10_correct_incorrect.png'
