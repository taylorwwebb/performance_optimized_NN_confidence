...
def save_params(args, model, epoch, selected_classes):
	# Directory
	...
	classes_fname = run_dir + 'epoch' + str(epoch) + '_classes.npz'
	np.savez(classes_fname, selected_classes=selected_classes)
...
def main():
	...
	parser.add_argument('--noise_range', type=list, default=[0.4, 0.8])
	...
	log.info('Randomly selecting 2 out of 10 classes...')
	...
	log.info(class_names[selected_classes])
	# Subset dataset to only these classes
	log.info('Subsetting...')
	# Training set
	s1 = train_set.targets == selected_classes[0]
	s2 = train_set.targets == selected_classes[1]
	train_set.targets = np.array(train_set.targets)[np.logical_or(s1,s2)]
	train_set.data = np.array(train_set.data)[np.logical_or(s1,s2)]
	...
	# Save model
	save_params(args, model, epoch, selected_classes)
