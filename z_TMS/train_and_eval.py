...
def test(args, encoder, class_out, conf_out,
	...
	for batch_idx, (data, target) in enumerate(loader):
		...
		# Get model predictions
		z = encoder(x, device)
		z += torch.randn(z.shape).to(device) * TMS
		y_pred = class_out(z).squeeze()
...
def main():
	...
	parser.add_argument('--signal_test', type=float, default=0.55)
	...
	for t in range(TMS_vals.shape[0]):
		trial_y_pred, trial_y_targ, trial_conf = test(args, encoder, class_out, conf_out, device, test_loader, signal=args.signal_test, noise=args.noise_test, TMS=TMS_vals[t])
