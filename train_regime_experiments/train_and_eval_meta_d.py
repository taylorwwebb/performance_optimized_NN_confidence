...
def train(args, encoder, class_out, conf_out, 
	...
	train_prog_dir = './train_prog_meta_d/'
...
def test(args, encoder, class_out, conf_out, device, loader, test_regime='standard'):
	...
	for batch_idx, (data, target) in enumerate(loader):
		...
		# Scale signal
		if test_regime == 'standard' or test_regime == 'fixed_sigma_mu_ratio' or test_regime == 'fixed_sigma':
			signal = ((torch.rand(x.shape[0]) * (args.signal_range[1] - args.signal_range[0])) + args.signal_range[0]).to(device)
			x = x * signal.view(-1, 1, 1, 1)
		elif test_regime == 'fixed_mu':
			x = x * args.signal_val
		# Scale to [-1, 1]
		x = (x - 0.5) / 0.5
		# Add noise
		if test_regime == 'standard' or test_regime == 'fixed_mu':
			noise = (torch.rand(x.shape[0]) * (args.noise_range[1] - args.noise_range[0])) + args.noise_range[0]
			noise = noise.view(-1, 1, 1, 1).repeat(1, 1, x.shape[2], x.shape[3])
			noise = (torch.randn(x.shape) * noise).to(device)
		elif test_regime == 'fixed_sigma_mu_ratio':
			noise = signal * args.signal_noise_ratio
			noise = noise.view(-1, 1, 1, 1).repeat(1, 1, x.shape[2], x.shape[3])
			noise = (torch.randn(x.shape)).to(device) * noise
		elif test_regime == 'fixed_sigma':
			noise = torch.ones(x.shape[0]) * args.noise_val
			noise = noise.view(-1, 1, 1, 1).repeat(1, 1, x.shape[2], x.shape[3])
			noise = (torch.randn(x.shape)).to(device) * noise.to(device)
		x = x + noise
	...
	return all_trial_y_pred, all_trial_y_targ, all_trial_conf
...
def main():
	...
	class_out = Class_out_binary(args).to(device)
	...
	# Get meta-d' for all test regimes
	test_regimes = ['standard', 'fixed_sigma_mu_ratio', 'fixed_sigma', 'fixed_mu']
	all_regimes_meta_d = {}
	for r in range(len(test_regimes)):
		trial_y_pred, trial_y_targ, trial_conf = test(args, encoder, class_out, conf_out, device, test_loader, test_regime=test_regimes[r])
		test_meta_d = compute_sensitivity(trial_y_pred, trial_y_targ, trial_conf)
		log.info('[Regime = ' + test_regimes[r] + '] ' + \
			 	 "[meta-d' = "+ '{:.2f}'.format(test_meta_d) + ']')
		all_regimes_meta_d[test_regimes[r]] = test_meta_d
	# Save results
	test_dir = './meta_d_test/'
	check_path(test_dir)
	test_dir += args.train_regime + '/'
	check_path(test_dir)
	model_dir = test_dir + 'run' + str(args.run) + '/'
	check_path(model_dir)
	np.savez(model_dir + 'all_test_regimes_meta_d.npz',
			 standard_meta_d=all_regimes_meta_d['standard'],
			 fixed_sigma_mu_ratio_meta_d=all_regimes_meta_d['fixed_sigma_mu_ratio'],
			 fixed_sigma_meta_d=all_regimes_meta_d['fixed_sigma'],
			 fixed_mu_meta_d=all_regimes_meta_d['fixed_mu'])
