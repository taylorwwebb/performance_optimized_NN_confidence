...
def test(args, encoder, pos, train_s1_dist, train_s2_dist, device, loader, s1_signal=0.5, s2_signal=1.0, noise=0.0):
	...
	# Report
	log.info('[s2 signal = ' + '{:.2f}'.format(s2_signal) + '] ' + \
			 "[d' = " + '{:.2f}'.format(d_prime) + ']')
	return d_prime
...
def main():
	...
	parser.add_argument('--signal_range_test', type=list, default=[0.001,1.0])
	parser.add_argument('--signal_N_test', type=int, default=1000)
	...
	for s in range(signal_test_vals.shape[0]):
		d = test(args, encoder, pos, train_s1_dist, train_s2_dist, device, test_loader, s1_signal=s1_signal, s2_signal=signal_test_vals[s], noise=args.noise_val_test)
		all_d.append(d)
	model_dir += 'run' + str(args.run) + '/'
	check_path(model_dir)
	np.savez(model_dir + 'i1245_dprime_results.npz',
