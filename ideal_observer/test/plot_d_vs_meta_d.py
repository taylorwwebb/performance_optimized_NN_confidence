...
def plot(args):
	...
	# Load model results
		...
		if args.model_behavior == 'model':
			if args.conf_noise == 0.0:
				fname = 'type1_vs_type2_sensitivity_results.npz'
			else:
				fname = 'type1_vs_type2_sensitivity_results_conf_noise_' + str(args.conf_noise) + '.npz'
		elif args.model_behavior == 'model_conf_noise_fit':
			conf_noise_fit = np.load(model_dir + 'conf_noise_fit.npz')['conf_noise']
			fname = 'type1_vs_type2_sensitivity_results_conf_noise_' + str(conf_noise_fit) + '.npz'
		...
		for r in range(args.N_runs):
			...
			try:
				...
				print('could not load data for run ' + str(r+1))
		# Convert to arrays
	...
	if args.model_behavior == 'behavior':
		...
		y_tick_labels = ['0','0.5','1','1.5','2','2.5']
		...
		y_max = 2.6
		y_ticks = [0,0.5,1,1.5,2,2.5]
		y_tick_labeks = ['0','0.5','1','1.5','2','2.5']
	# Generate plot
	ax = plt.subplot(111)
	...
	# Title
	plt.title('Type-1 vs. type-2 sensitivity', fontsize=title_fontsize)
	# Save
	if args.model_behavior == 'behavior':
		plt.title('Behavior', fontsize=title_fontsize)
		plot_fname = './ideal_observer_d_vs_meta_d_behavior.png'
	elif args.model_behavior == 'model':
		plt.title('Model, ' + r'$\xi$' + ' = ' + str(args.conf_noise), fontsize=title_fontsize)
		plot_fname = model_dir + 'ideal_observer_d_vs_meta_d_conf_noise_' + str(args.conf_noise) + '.png'
	elif args.model_behavior == 'model_conf_noise_fit':
		# plt.title('Model', fontsize=title_fontsize)
		plot_fname = model_dir + 'ideal_observer_d_vs_meta_d_model_conf_noise_fit.png'
