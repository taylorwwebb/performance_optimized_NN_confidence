...
def plot(args):
	...
	for r in range(args.N_runs):
		...
		# Load test results
		test_results = np.load(run_dir + 'i1245_dprime_results.npz')
	...
	plt.savefig(model_dir + 'i1245_mu_vs_d_prime.png')
	plt.close()
	# Save target signal
	np.savez(model_dir + 'i1245_fit.npz', targ_d_prime=targ_d_prime, estimated_mu=np.array(all_targ_signal))
