...
def plot(args):
	# Model directory
	model_dir = './test/' + args.train_regime + '_training/'
	# Get results
	all_runs_d = []
	all_runs_meta_d = []
	all_runs_meta_d_rS1 = []
	...
	for r in range(args.N_runs):
		...
		# Load test results
		test_results = np.load(run_dir + 'type1_vs_type2_sensitivity_results.npz')
		...
		# Save
		conf_noise_fname = run_dir + 'type1_vs_type2_sensitivity_results_conf_noise_' + str(args.conf_noise) + '.npz'
	...
	mse_fname = model_dir + 'meta_d_mse_noise_' + str(args.conf_noise) + '.npz'
...
def main():
	...
	parser.add_argument('--conf_noise', type=float, default=0.9)
