...
def plot(args):
	for r in range(args.N_runs):
		...
		for i in plot_cond:
			...
			ax = plt.subplot(111)
			plt.fill(np.append(np.insert(x_range, 0, x_range[0]), x_range[-1]), np.append(np.insert(abs_pc1_correct_dist, 0, 0), 0), color='lightgreen', alpha=0.5)
			plt.fill(np.append(np.insert(x_range, 0, x_range[0]), x_range[-1]), np.append(np.insert(abs_pc1_incorrect_dist, 0, 0), 0), color='slateblue', alpha=0.5)
			plt.xlabel('|Principal component 1|', fontsize=args.axis_fontsize)
			plt.xlim([0,17])
