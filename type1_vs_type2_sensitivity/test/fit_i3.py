...
def plot(args):
	...
	for r in range(args.N_runs):
		...
		run_dir = './run' + str(r+1) + '/'
		# Load test results
		test_results = np.load(run_dir + 'i3_d_prime_results.npz')
	...
	plt.savefig('./i3_mu_vs_d_prime.png')
	plt.close()
	# Save target signal
	np.savez('./i3_fit.npz', targ_d_prime=targ_d_prime, estimated_mu=targ_signal)
