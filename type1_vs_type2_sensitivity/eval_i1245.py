...
def main():
	...
	for s in range(signal_test_vals.shape[0]):
		y_pred, y_targ = test(args, encoder, class_out, device, test_loader, s1_signal=s1_signal, s2_signal=signal_test_vals[s], noise=args.noise_val_test)
		d = calc_d_prime(y_pred, y_targ)
		log.info('[s1 signal = ' + '{:.2f}'.format(s1_signal) + '] ' + \
				 '[s2 signal = ' + '{:.2f}'.format(signal_test_vals[s]) + '] ' + \
			 	 '[d-prime = ' + '{:.2f}'.format(d) + ']')
		all_d.append(d)
	# Save test results
	test_dir = './test/'
	...
	np.savez(model_dir + 'i1245_d_prime_results.npz',
